...
public class Main extends JPanel implements KeyListener, ActionListener {
	...
	//2D cross product
	double edgeFunction(double[] v1, double[] v2, double[] p) { return (p[0] - v1[0]) * (v2[1] - v1[1]) - (p[1] - v1[1]) * (v2[0] - v1[0]); }
	double edgeFunction(Vector2 v1, Vector2 v2, Vector2 p) { return (p.x - v1.x) * (v2.y - v1.y) - (p.y - v1.y) * (v2.x - v1.x); }
	double edgeFunction(double v1x, double v1y, double v2x, double v2y, double px, double py) { return (px - v1x) * (v2y - v1y) - (py - v1y) * (v2x - v1x); }
