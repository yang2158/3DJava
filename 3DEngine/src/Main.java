...
//*/
	...
	public void paintComponent(Graphics g) {
			...
			for (int objID = 0; objID < list.size();objID++) {
				...
				if( list.get(objID).Cords.length ==3) {
					...
					for (j=0; j < list.get(objID).Cords.length; j++) {
						...
						list.get(objID).camCords[j]= quaternion.rotate(CPos, list.get(objID).Cords[j], quaternion.degToRadian(-COre.x), quaternion.degToRadian(COre.y));
		...
		displayBack();g2.drawImage(displayBuffer, 0, 0, null);
	...
	public Vector3 offScreen(Vector3 point, Vector3 other) {
